...
local Heritage = {
    ...
    BackgroundPlanet = { Dictionary = "planet", Texture = "planet", Width = 431, Height = 228 },
...
function RageUI.PlanetPreview()
    ---@type table
    local CurrentMenu = RageUI.CurrentMenu;
    if CurrentMenu ~= nil then
        if CurrentMenu() then
            RenderSprite(Heritage.BackgroundPlanet.Dictionary, Heritage.BackgroundPlanet.Texture, CurrentMenu.X, CurrentMenu.Y + CurrentMenu.SubtitleHeight + RageUI.ItemOffset, Heritage.BackgroundPlanet.Width + (CurrentMenu.WidthOffset / 1), Heritage.BackgroundPlanet.Height)
